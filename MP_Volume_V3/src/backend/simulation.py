...
from .ion_species_config_container import IonSpeciesConfig
from .ion_channel_config_container import IonChannelConfig
...
class Simulation(Trackable):
    ...
    def __init__(self,
                 *,
                 ...
                 ion_species_config: IonSpeciesConfig = None,
                 ion_channel_config: IonChannelConfig = None,
        ...
        # Default configs
        self.ion_species_config = ion_species_config if ion_species_config is not None else IonSpeciesConfig
        self.ion_channel_config = ion_channel_config if ion_channel_config is not None else IonChannelConfig
    ...
    def _initialize_species_and_channels(self):
        """
        Initialize ions and channels and link them based on the IonAndChannelsLink configuration.
        """
        # Step 1: Initialize ion species
        species_dict = {name: getattr(self.ion_species_config, name) for name in dir(self.ion_species_config) if not name.startswith("__")}

        # Step 2: Initialize ion channels
        channel_dict = {name: getattr(self.ion_channel_config, name) for name in dir(self.ion_channel_config) if not name.startswith("__")}

        # Step 3: Connect channels to species
        for species_name, links in self.ion_channel_links.get_links().items():
            primary_species = species_dict[species_name]
            for channel_name, secondary_species_name in links:
                channel = channel_dict[channel_name]
                secondary_species = species_dict.get(secondary_species_name)
                primary_species.connect_channel(channel=channel, secondary_species=secondary_species)

        # Step 4: Add species to the simulation
