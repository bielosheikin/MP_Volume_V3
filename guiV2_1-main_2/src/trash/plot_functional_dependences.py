...
def plot_dependency():
    ...
    axes[0, 0].plot(pH_axis, pH_values_ClC, color="#348ABD", linewidth=1)
    ...
    axes[0, 0].set_ylabel(
        "pH-dependency function", fontname="Arial", fontsize=24, labelpad=25
    )
    axes[0, 0].set_xlabel("pH", fontname="Arial", fontsize=18)
    axes[0, 0].set_xticks(np.arange(2, 13, 2.0))

    (line,) = axes[0, 1].plot(
        pH_axis, pH_values_ASOR_wt, color="#348ABD", linewidth=1, label="ASOR wild-type"
    )
    (line,) = axes[0, 1].plot(
        pH_axis,
        pH_values_ASOR_mutant,
        color="#E24A33",
        linewidth=1,
        label="pH-shifted ASOR mutant",
    )
    ...
    axes[0, 1].legend(
        loc=1, bbox_to_anchor=(0.8, 0.5, 0.5, 0.5), fontsize=14, frameon=False
    )

    axes[1, 0].plot(voltage_axis * 1000, U_values_ClC, color="#348ABD", linewidth=1)
    ...
    axes[1, 0].set_ylabel(
        "U-dependency function", fontname="Arial", fontsize=24, labelpad=25
    )
    axes[1, 0].set_xlabel("U, mV", fontname="Arial", fontsize=18)

    axes[1, 1].plot(voltage_axis * 1000, U_values_ASOR, color="#348ABD", linewidth=1)
