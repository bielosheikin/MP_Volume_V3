...
from . import ionic_fluxes as i_flux
from . import nernst_potentials as P
...
def update_euler(past_state, int_step, derivative):
    ...
    return past_state + (
        int_step * derivative
    )  # Euler integration i.e. x(t) = x(t-1) + (dt * dx(t)/dt)


def run_simulation(initial_state_ions_amounts, parameters, **kwargs):
    """
    Arguments
    ==========
    ...
    flux_history_dictionary = {
        "ClC_ASOR": np.zeros(len(time_axis) - 1),
    ...
    dependency_histories = {
        "pH_ASOR": np.zeros(len(time_axis)),
    ...
    dependency_histories["pH_ASOR"][0] = dep_funct.pH_dependence_ASOR(
        parameters["pH_i"],
        pH_k2=parameters["ASOR_pH_k2"],
        pH_half=parameters["ASOR_pH_half"],
    )
    dependency_histories["v_ASOR"][0] = dep_funct.v_dependence_ASOR(
        parameters["U0"], U_k2=parameters["ASOR_U_k2"], U_half=parameters["ASOR_U_half"]
    )
    dependency_histories["pH_CLC"][0] = dep_funct.pH_dependence_ClC(
        parameters["pH_i"],
        pH_k2=parameters["CLC_pH_k2"],
        pH_half=parameters["CLC_pH_half"],
    )
    dependency_histories["v_CLC"][0] = dep_funct.v_dependence_ClC(
        parameters["U0"], U_k2=parameters["CLC_U_k2"], U_half=parameters["CLC_U_half"]
    )
    ...
    for t in tqdm(range(1, len(time_axis))):
        ...
        V_t[t] = (
            ...
            * (
                ...
                + ions_t[Na_idx, t]
                + ions_t[K_idx, t]
                + abs(parameters["X_amount"])
            )
        ) / parameters["Sum_initial_amounts"]
        if pb:
            perc = t / len(time_axis)
            print(perc)  # without printing progress bar works incorrectly
            pb.progress(perc)
    ...
    results = {}
    results["internal_ions"] = {
        "Na": {
            "amounts": ions_t[Na_idx, :],
            "concentrations": ions_t_concen[Na_idx, :],
        },
        "H": {"amounts": ions_t[H_idx, :], "concentrations": ions_t_concen[H_idx, :]},
        "Cl": {
            "amounts": ions_t[Cl_idx, :],
            "concentrations": ions_t_concen[Cl_idx, :],
        },
        "K": {"amounts": ions_t[K_idx, :], "concentrations": ions_t_concen[K_idx, :]},
    }
    results["fluxes"] = {
        "Cl": {
            "ASOR": flux_history_dictionary["ClC_ASOR"],
            "CLC": flux_history_dictionary["ClC_CLC"],
        },
        "Na": {
            "TPC": flux_history_dictionary["Na_TPC"],
            "NHE": flux_history_dictionary["Na_NHE"],
        },
        "H": {
            "CLC": flux_history_dictionary["H_Cl"],
            "NHE": flux_history_dictionary["H_NHE"],
            "vATPase": flux_history_dictionary["H_vATPase"],
            "H_leak": flux_history_dictionary["H_leak"],
        },
        "K": {"K": flux_history_dictionary["K"]},
    }
    results["other_variables"] = {
        "dependencies": {
            "pH_ASOR": dependency_histories["pH_ASOR"],
            "v_ASOR": dependency_histories["v_ASOR"],
            "pH_CLC": dependency_histories["pH_CLC"],
            "v_CLC": dependency_histories["v_CLC"],
            "t_vATPase": dependency_histories["t_vATPase"],
        },
        "vesicle_parameters": {
            "pH": dependency_histories["pH_t"],
            "V": V_t,
            "A": dependency_histories["A_t"],
            "C": dependency_histories["C_t"],
            "buffer_capacity": dependency_histories["buffer_capacity_t"],
            "U": dependency_histories["U"],
        },
    }
